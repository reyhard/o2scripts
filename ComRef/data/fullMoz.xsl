...
<xsl:template match="/commandreference">
	...
	<xsl:apply-templates select="document('class.xml')"/>
...
<xsl:template match="commandlist">
	<xsl:apply-templates/> 	<!-- Cannot sort in index, leave original order also here to avoid mismatches. -->
	<hr/>
</xsl:template>
...
<xsl:template match="category">
	<br/><br/><b>Category: </b><xsl:value-of select="."/>
</xsl:template>
