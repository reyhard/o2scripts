...
<xsl:template match="f">
	<a>
		<xsl:attribute name="href">
			fullMoz.xml#<xsl:value-of select="."/>
		</xsl:attribute>
	<xsl:apply-templates/>
	</a>
</xsl:template>
...
<xsl:template match="/">
	<HTML>
	<HEAD>
		<TITLE><xsl:value-of select="document('comref.xml')/comref/commandlist/name"/></TITLE>
	</HEAD>
	<BODY bgcolor="#ffffff">
...
<!-- Command index -->
		...
		<p>
		<xsl:for-each select="document('comref.xml')/comref/commandlist/command">
...
<!-- Type reference -->
		...
		<p>
		<xsl:for-each select="document('comref.xml')/comref/typelist/type">
			<xsl:sort select="name"/>
			<a>
				<xsl:attribute name="href">
				fullMoz.xml#t_<xsl:value-of select="name"/></xsl:attribute>
				<xsl:value-of select="name"/>
			</a>
			<br/>
		</xsl:for-each>
...
<!-- Array reference -->
		...
		<p>
		<xsl:for-each select="document('comref.xml')/comref/arraylist/array">
			<xsl:sort select="name"/>
			<a>
				<xsl:attribute name="href">
				fullMoz.xml#ar_<xsl:value-of select="name"/></xsl:attribute>
				<xsl:value-of select="name"/>
			</a>
			<br/>
		</xsl:for-each>
...
<!-- Scripting topics -->
		...
		<p>
		<xsl:for-each select="document('script.xml')/scripting/topic">
			<xsl:sort select="name"/>
			<a>
				<xsl:attribute name="href">
				fullMoz.xml#scr_<xsl:value-of select="name"/></xsl:attribute>
				<xsl:value-of select="name"/>
			</a>
			<br/>
		</xsl:for-each>
...
<!-- Class names -->
		...
		<p>
		<xsl:for-each select="document('class.xml')/classlist/class">
			<xsl:sort select="name"/>
			<a>
				<xsl:attribute name="href">
				fullMoz.xml#c_<xsl:value-of select="name"/></xsl:attribute>
